...
const FailureByZoneChart: React.FC<{ data: LuminaireEvent[] }> = ({ data }) => {
    const chartData = useMemo(() => {
        ...
        const zoneCounts = failures.reduce((acc, event) => {
            acc[event.zone] = (acc[event.zone] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        return Object.entries(zoneCounts).map(([name, value]) => ({ name, fallas: value }));
    }, [data]);
        ...
        <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer>
                <BarChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 5 }}>
                    ...
                    <Bar dataKey="fallas" name="Número de Fallas" fill="#818CF8">
